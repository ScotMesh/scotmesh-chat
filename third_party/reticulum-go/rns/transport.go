...
type LocalDestination struct {
	...
	// LinkHooks, when set, are installed on every link a peer opens to
	// this destination. See LinkHooks for what each receives and how they
	// interact with the manager-level handlers.
	LinkHooks *LinkHooks
...
func (t *Transport) handleLinkRequest(p *Packet) {
	...
	link, lrProof, fresh, err := t.linkManager.acceptIncomingLinkRequest(p, dest.Identity, nil /* signalling */, dest.LinkHooks)
	if err != nil {
	...
	if fresh && dest.LinkHooks != nil && dest.LinkHooks.OnEstablished != nil {
		dest.LinkHooks.OnEstablished(append([]byte(nil), link.ID...))
	}
}
...
func (t *Transport) handleLinkIdentify(p *Packet) {
	...
	h := t.linkManager.remoteIdentifiedHandler()
	if link.hooks != nil && link.hooks.OnIdentified != nil {
		h = link.hooks.OnIdentified
	}
	if h != nil {
...
// ErrLinkPayloadTooLarge is returned by SendOnLink for a payload that
// does not fit one link DATA packet. Split it, or send it as a Resource
// with SendResourceOverLink.
var ErrLinkPayloadTooLarge = errors.New("link send: payload exceeds LinkMDU")

// SendOnLink sends plaintext as one link DATA packet on an existing link,
// addressed by link_id. It is the send for the side that did not open the
// link — a server answering a client on the client's own link — which
// SendOverLink cannot do, because SendOverLink finds or opens a link by
// the responder's destination hash.
//
// The send does not wait for the peer's DATA proof: like upstream's
// Packet(link, data).send() without a receipt callback, delivery is best
// effort and a lost packet is the application protocol's to notice. It
// never opens a link. Errors: the link is unknown or not Active, the
// payload is larger than LinkMDU (ErrLinkPayloadTooLarge), or the
// broadcast failed (ErrLinkSendFailed).
func (t *Transport) SendOnLink(linkID, plaintext []byte) error {
	if len(plaintext) > LinkMDU {
		return fmt.Errorf("%w: %d > %d bytes", ErrLinkPayloadTooLarge, len(plaintext), LinkMDU)
	}
	link := t.linkManager.Get(linkID)
	if link == nil {
		return fmt.Errorf("link send: unknown link_id %x", linkID)
	}
	link.mu.Lock()
	if link.State != LinkActive {
		state := link.State
	...
	signing := append([]byte(nil), link.Signing...)
	encryption := append([]byte(nil), link.Encryption...)
	id := append([]byte(nil), link.ID...)
	link.mu.Unlock()

	pkt, err := BuildLinkDataPacket(id, signing, encryption, plaintext)
	if err != nil {
		return fmt.Errorf("link send: build DATA: %w", err)
	}
	if err := t.Broadcast(pkt); err != nil {
